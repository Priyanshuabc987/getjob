...
import { ArrowLeft, Briefcase, Plus, Trash2, Loader2, Check, X, Edit2 } from 'lucide-react';
...
export default function ManageExperiencePage() {
  ...
  const [form, setForm] = useState<Partial<ExperienceEntry>>({
  ...
  const handleSave = async (isNew: boolean = true, entryId?: string) => {
    ...
    try {
      if (!isNew && entryId) {
        const old = profile.experience.find(e => e.id === entryId);
      ...
      setIsAdding(false);
      setEditingId(null);
      setForm({ company: '', role: '', startDate: '', endDate: '', description: '', isCurrent: false });
        ...
        <div className="space-y-8">
              ...
              <CardContent className="p-6 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <Label className="text-[10px] uppercase font-bold text-muted-foreground">Company <span className="text-destructive">*</span></Label>
                    <Input value={form.company} onChange={e => setForm({...form, company: e.target.value})} className="h-12 rounded-xl" />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-[10px] uppercase font-bold text-muted-foreground">Role <span className="text-destructive">*</span></Label>
                    <Input value={form.role} onChange={e => setForm({...form, role: e.target.value})} className="h-12 rounded-xl" />
                  </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                ...
                <div className="flex gap-3 pt-4">
                  <Button onClick={() => handleSave(true)} disabled={loading} className="flex-1 h-14 rounded-full font-bold text-lg action-button-glow">
                    {loading ? <Loader2 className="animate-spin mr-2" /> : <><Check className="w-5 h-5 mr-2" /> Save Experience</>}
                  </Button>
          ...
          <div className="space-y-4">
            {profile.experience.map(exp => (
              <div key={exp.id}>
                {editingId === exp.id ? (
                  <Card className="rounded-3xl border-2 border-primary/20 shadow-xl bg-white dark:bg-card p-6 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        <Label className="text-[10px] uppercase font-bold">Company <span className="text-destructive">*</span></Label>
                        <Input defaultValue={exp.company} onChange={e => setForm({...form, company: e.target.value})} className="h-12 rounded-xl" />
                      </div>
                      <div className="space-y-2">
                        <Label className="text-[10px] uppercase font-bold">Role <span className="text-destructive">*</span></Label>
                        <Input defaultValue={exp.role} onChange={e => setForm({...form, role: e.target.value})} className="h-12 rounded-xl" />
                      </div>
                    </div>
                    <div className="flex gap-3">
                      <Button onClick={() => handleSave(false, exp.id)} disabled={loading} className="flex-1 h-12 rounded-full font-bold">Update</Button>
                      <Button onClick={() => handleDelete(exp)} disabled={loading} variant="destructive" className="h-12 rounded-full px-6"><Trash2 className="w-5 h-5" /></Button>
                  ...
                  <Card className="rounded-2xl border-none shadow-md bg-white dark:bg-card p-6 flex items-center justify-between group hover:shadow-lg transition-all">
                    ...
                    <Button variant="ghost" size="icon" onClick={() => { setEditingId(exp.id); setForm(exp); }} className="rounded-full text-muted-foreground hover:text-primary">
